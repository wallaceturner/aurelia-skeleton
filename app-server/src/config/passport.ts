...
passport.deserializeUser((id, done) =>{
  config.databaseRepository.getUserById(id)
  .then((user:any)=>{
    done(null, user);
  })
  .catch((err:any)=>{
...
passport.use(new LocalStrategy({ usernameField: 'email' }, async (email, password, done) => {
  let repository = config.databaseRepository;
  let [err,user] = await to(repository.getUser(email));  
  
  if (err) {
    return done(null, false, { message: `Server error: ${err}`});
  }
  if (!user) {
